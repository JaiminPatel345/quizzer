...
const PerformanceStatsSchema = new Schema<PerformanceStats>({
  ...
  totalTimeSpent: { type: Number, required: true, min: 0 }, // minutes
  consistency: { type: Number, required: true, min: 0, max: 100 } // percentage
}, { _id: false });
...
const PerformanceTrendsSchema = new Schema<PerformanceTrends>({
  improving: { type: Boolean, required: true },
  trendDirection: {
    type: String,
    enum: ['up', 'down', 'stable'],
    required: true
...
const TopicWiseStatsSchema = new Schema<TopicWiseStats>({
  ...
  avgTimePerQuestion: { type: Number, required: true, min: 0 } // seconds
}, { _id: false });
  ...
  recentPerformance: {
    ...
    validate: {
      validator: function(performances: RecentPerformance[]) {
        return performances.length <= 10;
      },
      message: 'Recent performance cannot exceed 10 entries'
    }
  },
...
// Indexes
PerformanceHistorySchema.index({ userId: 1, subject: 1, grade: 1 }, { unique: true });
